...
import gdx.bullets.utils.TiledObjectUtil;
import gdx.bullets.utils.CameraStyleUtil;
import gdx.bullets.SpriteExtended;
import gdx.bullets.utils.Lights;
...
import gdx.bullets.Bullet;
import gdx.bullets.utils.ContactListenerUtil;
...
public class ScrPlay extends ApplicationAdapter implements Screen {
    ...
    public ScrPlay(Game _game) {
        ...
        alBullets.add(new Bullet("pik.png", 400, 316, world, 2));
    ...
    public void render(float fDelta) {
        ...
        for (int i = 0; i < alBullets.size(); i++) {
            batch.draw(alBullets.get(i).getSprite(), alBullets.get(i).getX(), alBullets.get(i).getY());
        }
        batch.end();
    ...
    public void inputUpdate(float fDelta) {
        ...
        if (Gdx.input.isKeyJustPressed(Input.Keys.SPACE)) {
            //alBullets.add(new Bullet("pik.png", sprExtHero.getX() + sprExtHero.getSprite().getWidth(), sprExtHero.getY() + sprExtHero.getSprite().getHeight() / 2, world, 1));
            //alBullets.add(new Bullet("pik.png", sprExtHero.getX(), sprExtHero.getY() + sprExtHero.getSprite().getHeight(), world, 1)); //this creates a cool throwing look for the bullet
            if (sprExtHero.nDir == 1) {
                alBullets.add(new Bullet("pik.png", sprExtHero.getX(), sprExtHero.getY() + sprExtHero.getSprite().getHeight() / 2, world, sprExtHero.nDir));
            } else if (sprExtHero.nDir == 2) {
                alBullets.add(new Bullet("pik.png", sprExtHero.getX() + sprExtHero.getSprite().getWidth(), sprExtHero.getY() + sprExtHero.getSprite().getHeight() / 2, world, sprExtHero.nDir));
            }
        }
    ...
    public void bulletUpdate() {
        for (int i = 1; i < alBullets.size(); i++) {
            alBullets.get(i).move();
            //alBullets.get(i).fX = 
            alBullets.get(i).setPosition(alBullets.get(i).body.getPosition().x * fPPM - (alBullets.get(i).getSprite().getWidth() / 2), alBullets.get(i).body.getPosition().y * fPPM - (alBullets.get(i).getSprite().getHeight() / 2));
        }
    }
